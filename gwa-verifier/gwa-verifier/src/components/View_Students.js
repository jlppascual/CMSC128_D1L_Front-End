             ...
             <div className='view-student-body'>
                ...
                onChange={this.onHandleChange}/>
                 {this.state.record != []? this.state.record.map((record,i)=>{
                     return <span key={i}><div className='student-tile'>{i+1}. {record.last_name}, {record.first_name}, {record.middle_name} {record.suffix} {record.student_number} {record.degree_program} {record.gwa}</div></span>
                 }): ""}

                 {/* {this.state.ordervalue ==="name"? "name":"gwa"}  */}
             </div>
             <Footer/>
         </div>
         );
